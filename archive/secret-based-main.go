...
func loadConfig() (*Config, error) {
	config := &Config{
		// All configuration now comes from Kubernetes secrets
		ClusterDomain:     getSecretValue("CLUSTER_DOMAIN", "cluster.local"),
		ClusterName:       getSecretValue("CLUSTER_NAME", "openshift-cluster"),
	...
	// Parse exclude namespaces from secret
	excludeNS := getSecretValue("EXCLUDE_NAMESPACES", "")
	if excludeNS != "" {
	...
	if config.MinIOEndpoint == "" || config.MinIOAccessKey == "" || config.MinIOSecretKey == "" {
		return nil, fmt.Errorf("MinIO configuration is incomplete - missing endpoint, access key, or secret key")
	}

	log.Printf("Configuration loaded: Cluster=%s/%s, MinIO=%s, Bucket=%s, SSL=%v", 
		config.ClusterDomain, config.ClusterName, config.MinIOEndpoint, config.MinIOBucket, config.MinIOUseSSL)

	return config, nil
}

// getSecretValue gets configuration from Kubernetes secrets mounted as environment variables
func getSecretValue(key, defaultValue string) string {
	// First try to get from environment variable (Kubernetes secret mount)
...
func (cb *ClusterBackup) Run() error {
	...
	log.Printf("Starting backup run for cluster %s/%s", cb.config.ClusterDomain, cb.config.ClusterName)
	...
	// Get Kubernetes API discovery client
	kubeConfig, err := rest.InClusterConfig()
	if err != nil {
	...
	log.Printf("Found %d namespaces to backup", len(namespaces))
	...
	log.Printf("Backup run completed successfully")
	return nil
}
...
func (cb *ClusterBackup) cleanResource(resource *unstructured.Unstructured) map[string]interface{} {
	...
	// Clean spec section of runtime-specific values
